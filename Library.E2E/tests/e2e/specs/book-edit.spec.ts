...
test.describe('Book Edit page', () => {
    ...
    test('Edit page loads and shows book info', async ({ page, api }) => {
        // Arrange
    ...
    test('Shows validation error if title is empty', async ({ page, api }) => {
        ...
        // Assert
        await expect(edit.errorText).toContainText('Title and Genre are required');
    ...
    test('Shows validation error if copies is negative', async ({ page, api }) => {
        ...
        // Assert
        await expect(edit.errorText).toContainText('Available copies must be 0 or more');
